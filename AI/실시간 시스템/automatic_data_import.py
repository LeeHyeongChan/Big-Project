...
# start_date = datetime.strptime('2022-12-30', "%Y-%m-%d") #테스트
...
def MacroeconomicIndicators_day_update(start_date):
    ...
    stockai_db = pymysql.connect(host='3.35.230.74', user='root', password='1234', db='stockai', charset='utf8')
...
def NaverNews_day_update(start_date):
    ...
    naverdatapath="NaverNew_labeling105.csv"
    word=result.iloc[:1]
...
def invest_value_update(now_date):
    ...
    stockai_db = pymysql.connect(host='3.35.230.74', user='root', password='1234', db='stockai', charset='utf8', cursorclass=pymysql.cursors.DictCursor)
...
def short_model_create():
    conn = pymysql.connect(host='3.35.230.74', user='root', password='1234', db='stockai', charset='utf8', cursorclass=pymysql.cursors.DictCursor)
    curs = conn.cursor()
    ...
    try:
        ...
        for i in range(0,stock_n):
            ...
            joblib.dump(xgb,'./model/xgbboost/short_pred/'+name+'_model.pkl')
...
def long_model_create():
    ...
    try:
        ...
        for i in range(0,stock_n):
            ...
            joblib.dump(xgb,'./model/xgbboost/long_pred/'+name+'_model.pkl')
...
def ModelResult_update(start_date):
    ...
    try:
        for i in range(0,len(stock_code_list)):
            ...
            #xgbboost
            test_x = df_m[f_col][-11:]
            
            xgb_short_pred = joblib.load('./model/xgbboost/short_pred/'+name+'_model.pkl')
            #xgb_short_pred = pkl.load(open('./model/xgbboost/short_pred/'+name+'_model.pkl','rb'))
            xgb_short_y_pred = xgb_short_pred.predict(test_x)
            print("성공")
            xgb_long_pred = joblib.load('./model/xgbboost/long_pred/'+name+'_model.pkl')
            #xgb_short_pred = pkl.load(open('./model/xgbboost/long_pred/'+name+'_model.pkl','rb'))
            xgb_long_y_pred = xgb_long_pred.predict(test_x)

            print(name + "의 " + str(df_m['data_time'].tail(1))+" xgb_short_pred예측 결과 = ",xgb_short_y_pred[0]) #10일뒤 5일평균
            print(name + "의 " + str(df_m['data_time'].tail(1))+" xgb_long_pred예측 결과 = ",xgb_long_y_pred[0]) #30일뒤 10일평균
            print()
            ...
            lstm_short_model = load_model('./model/lstm/short_pred/'+name+'_model.h5')
            lstm_short_pred = lstm_short_model.predict(test_feature)
            
            lstm_long_model = load_model('./model/lstm/short_pred/'+name+'_model.h5')
            lstm_long_pred = lstm_long_model.predict(test_feature)
            
            print(name + "의 " + str(df_m['data_time'].tail(1))+" lstm_short_pred 예측 결과 = ",round(lstm_short_pred[0][0],5))
            print(name + "의 " + str(df_m['data_time'].tail(1))+" lstm_long_pred 예측 결과 = ",round(lstm_long_pred[0][0],5))
            print()
            size=len(df_m["data_time"])
            ...
            curs.execute(sql, (stock_code_list['stock_code'][i], df_m['data_time'][size-1], xgb_short_y_pred[0], xgb_long_y_pred[0], round(lstm_short_pred[0][0],5), round(lstm_long_pred[0][0],5)))
        conn.commit()
...
def AIRate_AIFunds_update(start_date):
    ...
    try:
        ...
        for i in range(0,stock_n):
            ...
            model = joblib.load('model/xgbboost/short_pred/'+name+'_model.pkl')
            ...
            #LSTM 예측
            model = load_model('model/lstm/short_pred/'+name+'_model.h5')
...
schedule.every(10).minutes.do(oik)
#일
schedule.every().sunday.at(run_time).do(short_model_create)
schedule.every().sunday.at(run_time).do(long_model_create)
